...
import { updateBoardTerm } from "@/actions/boards";
...
export function EditBoardTermDialog({
  boardTermId,
  defaultValues,
}: {
  boardTermId: string;
  defaultValues: BoardTermFormValues;
}) {
  ...
  async function onSubmit(values: BoardTermFormValues) {
    setSubmitError(null);
    try {
      await updateBoardTerm(boardTermId, values);
      setOpen(false);
    } catch (error) {
      setSubmitError(
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Edytuj kadencję zarządu</DialogTitle>
        </DialogHeader>
        <form
